...
format:
  revealjs:
    ...
    center: false
    hash: true
    footer: "Malnutrition L2 | Community & Family Medicine"
---
...
1. Classify differentiate protein-energy malnutrition
2. Recognise  micronutrient deficiencies 
3. Select appropriate tools for nutritional assessment (ABCD)
4. Outline management of SAM (WHO 2023)
...
## Marasmus vs Kwashiorkor {.smaller}
...
Two questions set the route: **appetite** (RUTF test) and **complications** (no appetite, severe oedema +++, infection, or age <6 months).

```{mermaid}
flowchart TD
  A[Child with SAM] --> B{Appetite OK and no complications?}
  B -->|Yes| C[Community care: outpatient + RUTF]
  B -->|No| D[Facility care: NRC / inpatient]
```

::: {.notes}
This decision tree is the single most useful slide for the case. Uncomplicated SAM with appetite goes home with RUTF; complicated is admitted. Our case (oedema + diarrhoea, infant) routes to the facility. ~3 min.
:::



# Facility-Based Management (NRC) 
...
## 10 Steps (4–6): Correct the Internal Milieu {.smaller}
...
# Micronutrient & Lifecycle Programmes 
...
- **Complicated?** Yes: oedema with intercurrent illness, plus infant considerations
- **Pathway:** complicated SAM, so refer to the **NRC**
  - Stabilise: **F-75** (no iron), treat infection, ReSoMal for dehydration
  - Rehabilitate: **F-100 / RUTF** for catch-up growth
  - Discharge on MUAC ≥ 12.5 cm, then follow up to prevent relapse
